...
import io.github.mpstudios56.cifra.adapter.CurrencyListAdapter;
...
public class CurrencyListActivity extends AbstractListActivity<Cursor> {
		...
		if (bPeriods != null) {
			...
			bPeriods.setOnClickListener(view -> {
				Intent intent = new Intent(CurrencyListActivity.this,
						io.github.mpstudios56.cifra.activity.PreferencesActivity2.class);
				intent.putExtra(io.github.mpstudios56.cifra.activity.PreferencesActivity2.SCREEN_EXTRA,
	...
	private void tidyCurrencies() {
		int folded = io.github.mpstudios56.cifra.db.Currencies.mergeSameCode(db.db());
		if (folded == 0) {
		...
		io.github.mpstudios56.cifra.utils.CurrencyCache.initialize(db);
