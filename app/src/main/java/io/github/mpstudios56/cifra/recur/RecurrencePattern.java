...
public class RecurrencePattern {
	...
	public void updateRRule(RRule r) {
		...
		case WEEKLY:
			...
			String[] a = days.split(",");
			for (String s : a) {
				RecurrenceViewFactory.DayOfWeek d = RecurrenceViewFactory.DayOfWeek.valueOf(s);
		...
		case MONTHLY:
			...
			RecurrenceViewFactory.MonthlyPattern pattern = RecurrenceViewFactory.MonthlyPattern.valueOf(state.get(RecurrenceViewFactory.P_MONTHLY_PATTERN+"_0"));
			switch (pattern) {
			case EVERY_NTH_DAY:
			...
			case SPECIFIC_DAY:
				...
				String[] x = s.split("-");
				RecurrenceViewFactory.SpecificDayPrefix prefix = RecurrenceViewFactory.SpecificDayPrefix.valueOf(x[0]);
				RecurrenceViewFactory.SpecificDayPostfix postfix = RecurrenceViewFactory.SpecificDayPostfix.valueOf(x[1]);
				int num = prefix == RecurrenceViewFactory.SpecificDayPrefix.LAST ? -1 : prefix.ordinal()+1;
				switch (postfix) {
					case DAY:
