...
import static io.github.mpstudios56.cifra.activity.CategorySelectorActivity.NO_SELECTED_ACCOUNT;
...
import io.github.mpstudios56.cifra.model.RestoredTransaction;
import io.github.mpstudios56.cifra.model.SmsTemplate;
import io.github.mpstudios56.cifra.model.SystemAttribute;
...
import io.github.mpstudios56.cifra.rates.ExchangeRatesCollection;
import io.github.mpstudios56.cifra.rates.HistoryExchangeRates;
import io.github.mpstudios56.cifra.rates.LatestExchangeRates;
...
public class DatabaseAdapter extends MyEntityManager {
    ...
    public long insertOrUpdateInTransaction(Transaction transaction, List<TransactionAttribute> attributes) {
        ...
        if (transaction.isNotTemplateLike()) {
            noteChange(DatabaseHelper.TRANSACTION_TABLE, transactionId,
                    ...
                    io.github.mpstudios56.cifra.sync.SyncPayload.of(db(), transactionId));
        }
        return transactionId;
    }
    ...
    public void noteRestoredForSharing(long transactionId, String title) {
        noteChange(DatabaseHelper.TRANSACTION_TABLE, transactionId, ChangeLog.INSERT,
                title, "", io.github.mpstudios56.cifra.sync.SyncPayload.of(db(), transactionId));
    }
    ...
    public void deleteTransactionNoDbTransaction(long id) {
        ...
        if (t.isNotTemplateLike()) {
            noteChange(DatabaseHelper.TRANSACTION_TABLE, id, ChangeLog.DELETE,
                    describeForTrash(t), whenForTrash(t),
                    io.github.mpstudios56.cifra.sync.SyncPayload.forDeletion(
    ...
    public ExchangeRate findRate(io.github.mpstudios56.cifra.model.Currency fromCurrency, io.github.mpstudios56.cifra.model.Currency toCurrency, long date) {
    ...
    public List<ExchangeRate> findRates(io.github.mpstudios56.cifra.model.Currency fromCurrency) {
    ...
    public Total getAccountsTotalInHomeCurrencyWithFilter(String filter) {
        io.github.mpstudios56.cifra.model.Currency homeCurrency = getHomeCurrency();
    ...
    public Total[] getAccountsTotalWithFilter(String filter) {
        ...
        Map<io.github.mpstudios56.cifra.model.Currency, Total> totalsMap = new HashMap<io.github.mpstudios56.cifra.model.Currency, Total>();
        for (Account account : accounts) {
            if (account.shouldIncludeIntoTotals()) {
                io.github.mpstudios56.cifra.model.Currency currency = account.currency;
    ...
    public void setDefaultHomeCurrency() {
        ...
        if (homeCurrency == Currency.EMPTY && singleCurrencyId > 0) {
            io.github.mpstudios56.cifra.model.Currency c = get(Currency.class, singleCurrencyId);
            c.isDefault = true;
            saveOrUpdate(c);
        }
    }
